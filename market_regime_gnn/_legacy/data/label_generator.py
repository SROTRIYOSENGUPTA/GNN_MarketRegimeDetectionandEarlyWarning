...
def classify_regimes(
    ...
    avg_corr = avg_corr.reindex(metrics.index).ffill().fillna(0.0)
...
def generate_market_labels(
    ...
    result = pd.DataFrame({
        ...
        "avg_corr": avg_corr_series.reindex(metrics.index).ffill().fillna(0.0),
    ...
    for ticker in stock_tickers:
        try:
            ...
            if isinstance(close, pd.DataFrame):
                close = close.squeeze()
            close = close.reindex(spy_close.index).ffill()
